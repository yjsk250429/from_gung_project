...
const ReviewSection = () => {
    ...
    const formatKSTDate = (isoString) => {
        ...
        const yy = offsetDate.getFullYear();
        ...
        <div className="reviews">
                    ...
                    {reviews.length > 0 ? (
                        reviews.map((review, idx) => (
                            <div key={review.id} className="tbody reviewList">
                                <span>{idx + 1}</span>
                                <span>{formatKSTDate(review.date)}</span>
                                <span
                                    className="detailBtn"
                                    onClick={() => {
            ...
            {mode === 'detail' && selectedReview && (
                <div className="detail">
                    <h3>리뷰 상세</h3>
                    <p>
                        <strong>번호:</strong> {selectedReview.id}
                    </p>
                    <p>
                        <strong>작성일:</strong> <span>{formatKSTDate(selectedReview.date)}</span>
                    </p>
                    {selectedReview.updatedAt && (
                        <p>
                            <strong>수정일:</strong> {selectedReview.updatedAt}
                        </p>
                    )}
                    <p>
                        <strong>카테고리:</strong> {selectedReview.category}
                    </p>
                    <p>
                        <strong>제목:</strong> {selectedReview.title}
                    </p>
                    <p>
                        <strong>내용:</strong> {selectedReview.reviewComment}
                    </p>
