...
const ReviewBlocks = () => {
  ...
  useLayoutEffect(() => {
    ...
    const init = () => {
      ...
      // ▼▼▼ 속도/강도 완화 설정 ▼▼▼
      engine.world.gravity.y = 4;   // 기본 1 → 더 약한 중력
      engine.timing.timeScale = 0.55;  // 전체 시뮬 느리게
      // ▲▲▲
      ...
      const ground = Bodies.rectangle(
        W / 2, H + wallThickness / 2, W, wallThickness, { isStatic: true }
      );
      const leftWall = Bodies.rectangle(
        -wallThickness / 2, H / 2, wallThickness, H * 5, { isStatic: true }
      );
      const rightWall = Bodies.rectangle(
        W + wallThickness / 2, H / 2, wallThickness, H * 5, { isStatic: true }
      );
