...
export default function TraditionClass() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const calcPinLen = () => window.innerHeight * (PANELS + 0.5);
    ...
    cardRefs.current.forEach((card, i) => {
      ...
      if (i === currentImageIndex - 1) {
        gsap.fromTo(
          card,
          { y: 2000, display: "none" },
          { y: 0, duration: 0.5, ease: "power3.out", display: "block", pointerEvents: "auto" }
        );
      } else {
        gsap.to(card, {
          y: -2000,
  ...
  return (
    <section className="section3-main" ref={sectionRef}>
      <div className="s3_main-title" ref={topMessageRef}>
        <h1>Korean<br/>Tradition Class</h1>
      </div>
      ...
      <div className="s3_card s3_card--top-right" ref={(el) => (cardRefs.current[0] = el)}>
        <div className="card-hover-image"><img src="/images/con3_hover_navi.png" alt="" /></div>
        <div className="card-content">
          <h3>차 향에<br/>머무는 시간</h3>
          <div className="card-tag">&lt;다도 체험&gt;</div>
          <p>온은한 차향이 번지는<br/>마음을 가다듬는 시간.<br/>다도의 깊 정에서선들의 예와 멋을<br/>함께 느껴보시길 바랍니다.</p>
        </div>
      </div>
      ...
      <div className="s3_card s3_card--bottom-left" ref={(el) => (cardRefs.current[1] = el)}>
        ...
        <div className="card-content">
          ...
          <p>고운 색감과 우아한 곡선을 간직한<br/>한복을 직접 입어보며,<br/>한국의 미와 전통을 몸소 느껴보는<br/>특별한 하루를 경험하세요.</p>
        </div>
      </div>
      ...
      <div className="s3_card s3_card--bottom-right" ref={(el) => (cardRefs.current[2] = el)}>
        ...
        <div className="card-content">
          ...
          <p>다섯 가지 빛깔의 재료가 어우러져<br/>한 그릇에 담기는 한국의 맛.<br/>직접 손으로 비벼며 만들어가는<br/>과정에서 한국 음식의 멋과 정을<br/>느껴 수 있습니다.</p>
        </div>
      </div>
