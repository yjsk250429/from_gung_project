...
const kindOf = (it) => {
  ...
  return ""; // 빈 문자열이면 링크에 넣지 않도록 처리 필요
};
...
export default function CuratedSageuk({ filterCategory }) {
    ...
    if (filterCategory && filterCategory !== "전체") {
      ...
      if (genreFilter) {
        out = out.filter((it) => {
          ...
          const genreMatch =
            isStringGenre && genreValue.toLowerCase() === genreFilter;
  ...
  if (curatedError)
    return <p style={{ color: "#f66" }}>에러: {curatedError}</p>;
    ...
    <div className="ottlist-container">
      <ul className="ottlist">
        {filtered.slice(0, visible).map((it) => {
          ...
          if (!type || !contentId) {
            // type 또는 contentId가 없으면 렌더링 하지 않음 (또는 fallback UI)
            return null;
          }
