...
const topItems = [
  "like never before ,", "unforgettable ,", "exclusive ,", "like never before ,", "unforgettable ,", "exclusive ,"
];

const bottomItems = [
  "지금까지 몰랐던 한국 , ", "지금 경험해보세요" , "지금까지 몰랐던 한국", "지금 경험해보세요"
];
    ...
    <div className="section4-main">
      ...
      <div className="marquee-container">
        <ul className="s4_top-text" ref={topRef}>
          ...
          {topItems.map((item, i) => <li key={`copy-${i}`}>{item}</li>)}
        </ul>
      </div>
      ...
      <div className="marquee-container">
        <ul className="s4_bottom-text" ref={bottomRef}>
          {bottomItems.map((item, i) => <li key={i}>{item}</li>)}
          {bottomItems.map((item, i) => <li key={`copy-${i}`}>{item}</li>)}
        </ul>
      </div>
    </div>
  );
