...
function computeCertString(data, mediaType) {
  ...
  // 이미 가공된 값이 있으면 먼저 사용
  const pre =
    normalizeCertToken(data.certification) || normalizeCertToken(data.cert);
  if (pre) return pre;
  ...
  if (mediaType === "tv") {
    ...
    if (Array.isArray(nodes) && nodes.length) {
      const pick = (cc) =>
        nodes.find((n) => (n?.iso_3166_1 ?? n?.country ?? n?.iso) === cc);
...
export default function OttDetail() {
    ...
    for (const it of bucket) {
      ...
      out.push({
        ...
        poster_path:
          ...
          "",
        backdrop_path:
          it?.backdrop_path ?? it?.backdrop ?? it?.media?.backdrop_path ?? "",
        profile_path: it?.profile_path ?? it?.profile ?? "",
        vote_average: it?.vote_average ?? it?.rating ?? it?.media?.vote_average,
        media_type:
          (it?.media_type || it?.media?.media_type || "").toLowerCase() || null,
      });
    }
    return out;
  }, [data]);
  ...
  if (ui.loading)
    return <div style={{ color: "#fff", padding: 24 }}>불러오는 중…</div>;
  if (ui.error)
    return <div style={{ color: "#f66", padding: 24 }}>에러: {ui.error}</div>;
  if (!data)
    return <div style={{ color: "#fff", padding: 24 }}>데이터 없음</div>;
