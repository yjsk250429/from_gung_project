...
const TourReviewDetail = () => {
    const { tourReviewOpen, closeTourReview, openTourReview, selectedReview, openWishModal } =
        useModalStore();
    ...
    const onEdit = () => {
        if (!isEditing) {
            ...
            // ✅ 스토어에서 최신 데이터 다시 불러오기
            const updatedUser = useAuthStore.getState().user;
            const refreshedReview = updatedUser.reviews?.find((r) => r.id === selectedReview.id);

            // ✅ 최신 리뷰로 다시 모달 갱신
            if (refreshedReview) {
                openTourReview(refreshedReview);
            }
            ...
            <div className="modal tourReviewDetail">
                <div className="top">
                    ...
                    <div className="name-wrap">
                        ...
                        <span className="date">
                            {selectedReview.date?.slice(0, 10)}
                            {selectedReview.updatedAt && ` (${selectedReview.updatedAt} 수정)`}
                        </span>
                        <p className="stars">
                            <span>
