...
function Stars({ rating = 0, className = "" }) {
  ...
  return (
    <p
      className={className}
      aria-label={`평점 ${safe10.toFixed(1)}점 (10점 만점)`}
    >
...
const pickFromArray = (arr, preferLang = "ko", size = "original") => {
  ...
  const normalized = arr.map((x) =>
    typeof x === "string"
      ? { url: x, lang: null }
      : {
          url: x.url ?? x.file_path ?? null,
          lang: x.lang ?? x.iso_639_1 ?? null,
        }
  );
  const chosen =
...
const normalizeCert = (cert) => {
  ...
  const getFromNode = (node) => {
    ...
    if (Array.isArray(arr)) {
      const found = arr.find((e) =>
        (e?.certification ?? e?.rating ?? "").toString().trim()
      );
  ...
  if (Array.isArray(cert)) {
    const pickBy = (cc) => {
      const node = cert.find(
        (n) => (n?.iso_3166_1 ?? n?.country ?? n?.iso) === cc
      );
    ...
    return (
      pickBy("KR") ||
      pickBy("US") ||
      cert.map(getFromNode).find(Boolean) ||
      fallback
    );
  }
...
const OttDetailVisual = ({
  ...
  const safeCert = useMemo(
    () => normalizeCert(cert ?? contentRating),
    [cert, contentRating]
  );
